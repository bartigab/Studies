...
$data = array( "last_post"=>($lastpost = $this->post->getLastItem())?$lastpost["date"]:"- brak wpisów -",
      ...
      if(isset($_GET['generateCaptha']))
      {
         $this->captcha->showCaptcha();
      }
             
if(isset($_POST['nazwaLogowania']) and $_POST['nazwaLogowania']!="" and isset($_POST['pass'])){
   if( !$this->login($_POST['nazwaLogowania'],$_POST['pass']) ){
      $data["error1"]=$this->error;
   }
}
if(isset($_POST['nazwaRejestracji']) and $_POST['pass1']!="" and ($_POST['pass1']==$_POST['pass2'])){
   if( !$this->captcha->checkCaptcha(strtoupper($_POST['captcha']))) {
      $data["error2"]="Zły kod captcha";
   }else{
      if( !$this->register($_POST['nazwaRejestracji'],$_POST['username'],$_POST['pass1']) )
