...
if (isset($_GET['cmd']) and $_GET['cmd']=='getpost') {
   if ($t = $this->post->get($_GET['postid'])) {
       echo json_encode($t, JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);
   } else {
       echo "<p>ERROR: Post ID: ".$_GET['postid']." not found!</p>\n";
   }  
   exit;
}

if (isset($_GET['cmd']) && $_GET['cmd'] == 'getimg') {
   if (isset($_GET['imgid'])) {
       if ($t = $this->image->get($_GET['imgid'])) {
           echo json_encode($t, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
       } else {
           echo json_encode(["error" => "Image ID not found!!"]);
       }
   } else {
       echo json_encode(["error" => "Image ID not specified!!"]);
   }
   exit;
}
...
if(isset($_GET['cmd']) and $_GET['cmd']=='delete' and ($this->u['userlevel']==10 or $this->u['userid']==$this->post->get($_GET['id'])['userid'])) {
...
if(isset($_GET['cmd']) and $_GET['cmd']=='edit' and ($this->u['userlevel']==10 or $this->u['userid']==$this->post->get($_GET['id'])['userid'])) {
   $data['post']=$this->post->get($_GET['id']);
   $this->reload(); 
}

if(isset($_GET['cmd']) and $_GET['cmd']=='imgdelete' and ($this->u['userlevel']==10 or $this->u['userid']==$this->image->get($_GET['imgid'])['userid'])) {
   $this->image->delete($_GET['imgid']);  
   $this->reload();  
}
