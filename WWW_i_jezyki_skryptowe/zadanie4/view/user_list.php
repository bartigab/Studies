<section><?php if($this->u['userlevel']==10){ ?>
<div>
<a href="?cmd=topics">Tematy</a>
<a href="?cmd=images">Obrazki</a>

<?php if($this->context=="topics"){?><a href="?cmd=userlist">Lista uczestników</a><?php } ?>
<?php if($this->context=="images"){?><a href="?cmd=userlist">Lista uczestników</a><?php } ?>

</div>
<?php } ?>
Zalogowany jako: <?=$this->u['username'];?> (<?=$this->u['userid'];?>) <a href="?cmd=logout" >Wyloguj się </a>

<?php if(!isset($_SESSION['userlist'])) $_SESSION['userlist']=false;
if($_SESSION['userlist'] and $this->context=="topics"){ ?>
<br />
<table><tr><th>Identyfikator</th><th>Nazwa</th><th>Poziom</th><th></th></tr>
<?php foreach($users as $k=>$v){ ?>
<tr>
<td><?=$v['userid']?></td>
<td><?=$v['username']?></td>
<td><?=($v['userlevel']==10)?'admin':'user';?></td>
<td><?php if($v['userid']!='admin'){ ?>
<a href="?cmd=changeuser&userid=<?=$v['userid']?>">Zmień</a>&nbsp;
<a class="danger" href="?cmd=deluser&userid=<?=$v['userid']?>">Kasuj</a>
<?php } ?></td>
</tr>
<?php } ?>
</table>
<?php } ?>
</section>
